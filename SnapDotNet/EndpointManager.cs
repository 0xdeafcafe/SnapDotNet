...
{
	/// <summary>
	/// Provides various methods for sending GET and POST requests to Snapchat servers.
	/// </summary>
	internal sealed class EndpointManager
	{
		/// <summary>
		/// Provides a <see cref="EndpointManager"/> for each endpoint library.
		/// </summary>
		...
		private EndpointManager(Uri baseUri)
		{
			BaseUri = baseUri;
		}
		...
		/// <param name="endpointName">The name of the endpoint.</param>
		/// <param name="data">A dictionary containing the data to include in the POST request.</param>
		/// <returns>Upon success, a <see cref="HttpResponseMessage"/> received from the server.</returns>
		...
		public async Task<HttpResponseMessage> PostAsync(string endpointName, Dictionary<string, string> data, string token, Dictionary<string, string> headers)
		{
			...
			// Send POST request to endpoint and return the response if it succeeded.
			Debug.WriteLine("[EndpointManager] Sending POST request to {0}", endpoint);
			...
			Debug.WriteLine("[EndpointManager] Invalid HTTP response (reason: {0})", response.ReasonPhrase);
		...
		/// <summary>
		/// Sends a GET request to an API endpoint and receives binary data from the server.
		/// </summary>
		/// <param name="endpointName">The name of the endpoint.</param>
		/// <returns>A byte array containing the data received from the server.</returns>
		public async Task<byte[]> GetAsync(string endpointName)
		{
			...
			byte[] data = (await response.Content.ReadAsBufferAsync()).ToArray();
			if (response.Content.Headers.ContentEncoding.Contains(HttpContentCodingHeaderValue.Parse("gzip")))
			{
		...
		public async Task<HttpResponseMessage> GetAsync(string endpointName, Dictionary<string, string> headers)
		{
			...
			// Send GET request to endpoint and return the response if it succeeded.
